...
-(void)loadView
{
    ...
    [scrollFlowLayout.widthSize myEqualTo:(@(MyLayoutSize.wrap))]; //设置布局视图的宽度由子视图包裹，当水平流式布局的这个属性设置为YES，并和pagedCount搭配使用会产生分页从左到右滚动的效果。
    [scrollFlowLayout.heightSize myEqualTo:(scrollView.heightSize)]; //因为是分页从左到右滚动，因此布局视图的高度必须设置为和父滚动视图相等。
